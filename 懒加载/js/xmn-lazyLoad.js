...
let lazyLoad = function (obj) {
  ...
  function isInSight (dom) {
    //获取dom距离窗口顶部的距离
    const bound = dom.getBoundingClientRect();

    const clientHeight = window.innerHeight;

    return bound.top <= clientHeight + 20;
  }

  function init() {
    if(scrollContent){
      let arrP = document.getElementsByClassName(scrollContent);
      for (let i = 0,len = arrP.length;i< len;i++){
        arrP[i].onscroll = function () {
          imgClass(el);
        }
      }
    }else{
